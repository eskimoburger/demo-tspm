  ...
  return (
    <>
      <div>
        ...
        <button
          onClick={() => {
            getProjectByName(props.projectName);
          }}
          className="focus:outline-none  bg-white hover:bg-blue-100 text-gray-800 font-semibold py-1 px-4 border  rounded shadow "
         
        >
         <span className="flex items-center gap-2" style={{fontSize:"18px"}}><i className="bx bxs-detail" style={{ fontSize: "24px" }}></i>แสดงรายละเอียด</span> 
        ...
        > 
           <span className="flex items-center gap-2" style={{fontSize:"18px"}}> <i className='bx bxs-x-square' style={{fontSize:"24px"}}  ></i> ปฏิเสธ</span>        
        </button>
      ...
      >
        ...
        <DialogContent dividers>
          ...
          <div className="bg-gray-100 break-words" style={{textIndent:"50px"}}>
            {/* <p
      ...
      <Dialog
        ...
        //onClose={handleClose}
      >
        <DialogTitle > <p className="text-2xl">{  status == 1 ? "ยืนยันการยอมรับหัวข้อโครงงาน ? "  : status == 2 ? "ยืนยันปฏิเสธหัวข้อโครงงาน ?" :null}</p> </DialogTitle>
        <DialogContent className=" text-center " >

        <i className='bx bx-error-circle bx-tada  m-2  ' style={{fontSize:"180px"}} ></i>      
        </DialogContent>
        <DialogActions  >
        <button onClick={()=>{setAlert(false)}}  className = " h-10 rounded-lg bg-red-600   uppercase font-semibold hover:bg-red-700 text-gray-100 transition px-4 ">ยกเลิก</button>
        <button onClick={()=>{
          if(status ==1)
          {finalConfirm(1,`${props.teacherName}  ได้ยอมรับหัวข้อโครงงาน ${props.projectName}`)}
          else if(status == 2){
           finalConfirm(2,`${props.teacherName}  ได้ปฏิเสธหัวข้อโครงงาน ${props.projectName}`)
          }   
          }}  className = " h-10 rounded-lg bg-gray-400   uppercase font-semibold hover:bg-gray-600 text-white transition px-4 ">ยืนยัน</button>
