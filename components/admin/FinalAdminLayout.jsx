...
export default function FinalAdminLayout({ children, title }) {
          ...
          <nav className=" text-xl space-y-2">
            ...
            <Link href="/students/profile">
              <a className="flex items-center gap-2 py-2.5 px-4 rounded transition duration-200 hover:bg-gray-200 hover:text-gray-800">
                <PersonIcon /> การจัดการโครงงาน
              </a>
            </Link>
            <Link href="/students/search">
              <a className="flex items-center gap-2 py-2.5 px-4 rounded transition duration-200 hover:bg-gray-200 hover:text-gray-800">
                <SearchIcon /> การแจ้งแตือน
              </a>
            </Link>
            <Link href="/students/document">
              <a className="flex items-center gap-2 py-2.5 px-4 rounded transition duration-200 hover:bg-gray-200 hover:text-gray-800">
                <DescriptionIcon /> ประวัตินิสิต
              </a>
            </Link>
        ...
        <div className=" flex flex-row h-full ">
          ...
          >
            ...
            <Link href={`/admins`}>
              <a
                className={` flex items-center gap-2 py-2.5 px-4 rounded transition duration-200 hover:bg-gray-200 hover:text-gray-800 ${
                  router.asPath === `/admins` ? "bg-gray-200" : ""
                }`}
              >
                <HomeIcon style={{color:"red"}} /> หน้าแรก
              </a>
            </Link>

            <Link href={`/admins/addteacher`}>
              <a
                className={` flex items-center text-lg gap-2 py-2.5 px-4 rounded transition duration-200 hover:bg-gray-200 hover:text-gray-800 ${
                  router.asPath === `/admins/addteacher`
                    ? "bg-gray-200"
                    : ""
                }`}
              >
               <ListAltIcon/> เพิ่มรายชื่ออาจารย์
              </a>
            </Link>
            <Link href={`/admins/addstudent`}>
              <a
                className={`text-lg   flex items-center gap-2 py-2.5 px-4 rounded transition duration-200 hover:bg-gray-200 hover:text-gray-800 ${
                  router.asPath === `/admins/addstudent`
                    ? "bg-gray-200"
                    : ""
                }`}
              >
                <NotificationsActiveIcon className="text-yellow-500"   /> เพิ่มรายชื่อนักเรียน
              </a>
            </Link>
